...
private:
	...
	/**
	 * print_raw functions : send raw strings
	 *
	 */

	template<class T>
	static inline void print_raw(T val) {
		char buffer[10];
		ltoa(val, buffer, 10);
		print_raw((const char *) buffer);
	}

	static inline void print_raw(bool val) {
		(val) ? print_raw("true") : print_raw("false");
	}

	static inline void print_raw(char* val) {
		for (uint16_t i = 0; i < strlen(val); i++) {
			send_char(val[i]);
		}
	}

	static inline void print_raw(const char* val) {
		for (uint16_t i = 0; i < strlen(val); i++) {
			send_char(val[i]);
		}
	}

	static inline void print_raw(float value, int places) {
		int digit;
		float tens = 0.1;
		int tenscount = 0;
		int i;
		...
		// print_raw out the negative if needed
		if (value < 0)
			print_raw("-");
		...
		// otherwise, print_raw the point and continue on
		print_raw(".");

		// now print_raw out each decimal place by shifting digits one by one into the ones place and writing the truncated value
		for (i = 0; i < places; i++) {
			tempfloat *= 10.0;
			digit = (int) tempfloat;
			print_raw(digit);
...
public:
	...
	/**
	 * Raw bytes writing/reading functions
	 */

	template<class T>
	static inline void write_bytes(T val) {
		uint8_t bytes[4];
	...
	template<class T>
	static inline uint8_t read_bytes(T &val, uint16_t timeout = 0) {
		uint8_t buffer[4];

		for (unsigned int i = 0; i < sizeof(T); i++) {
			if (read_char(buffer[i], timeout) == READ_TIMEOUT)
				return READ_TIMEOUT;
		}
