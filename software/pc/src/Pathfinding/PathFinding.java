...
public class PathFinding
{
	...
	public Path computePath(Point start, Point end)
	{
    	...
    	{
	    	if( PathFinding.intersects(start, end, m_table.getLines().get(ind_ligne).getA(), m_table.getLines().get(ind_ligne).getB()))
	    	{
	    		node = PathFinding.intersection(start, end, m_table.getLines().get(ind_ligne).getA(), m_table.getLines().get(ind_ligne).getB());
	    		intersects = true;
	...
	public Path simplify(Path path)
	{
		for(int i = 0; i < path.size() - 2; i++)
		{
			...
			for(int ind_ligne = 0 ; ind_ligne < m_table.getLines().size() ; ind_ligne++)
	    	{
		    	if( PathFinding.intersects(path.getPosition(i), path.getPosition(i+2), m_table.getLines().get(ind_ligne).getA(), m_table.getLines().get(ind_ligne).getB()));
		    	{
		    		removable = false;
		    	}
	    	}
